...
- [ ] `2026-07-20T18:39:13Z` — `codex` — A fresh `pnpm install --frozen-lockfile` installs dependencies but exits nonzero with `ERR_PNPM_IGNORED_BUILDS` because required native build scripts are not recorded in the repository policy. Commit the approved/ignored build-script policy so a clean install is non-interactive and succeeds.
